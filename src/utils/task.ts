  ...
  public async run(data: TranslateTask) {
    ...
    if (!data.langfrom || !data.langto) {
      ...
      const { fromLanguage, toLanguage, isInferred } = autoDetectLanguage(
        Zotero.Items.get(data.itemId || -1) as unknown as Zotero.Item,
      );
...
export function addTranslateTitleTask(
  ...
  const addon = Zotero[config.addonInstance] as Addon;
  const ztoolkit = addon.data.ztoolkit;
  const item = Zotero.Items.get(itemId) as unknown as Zotero.Item | false;
  if (
    item &&
    item.isRegularItem() &&
    !(
      skipIfExists &&
      ztoolkit.ExtraField.getExtraField(item as Zotero.Item, "titleTranslation")
    )
  ) {
...
export function addTranslateAbstractTask(
  ...
  if (
    ...
    !(
      skipIfExists &&
      ztoolkit.ExtraField.getExtraField(item as Zotero.Item, "abstractTranslation")
    )
  ) {
