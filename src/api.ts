...
async function translate(
  ...
  if (!isDeprecatedCall && !(serviceOrOptions as any).pluginID) {
    throw `[ZoteroSci:api.translate] pluginID is required since 1.1.0-23. Please contact the plugin developer for more information.`;
  }

  if (isDeprecatedCall) {
    Zotero.warn(
      new Error(
        "[ZoteroSci:api.translate] This call is deprecated. Please use `translate(raw, options)` instead.",
      ),
    );
  }
