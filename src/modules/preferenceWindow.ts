...
function buildPrefsPane() {
  ...
  doc
    .querySelector(`#${makeId("enableAutoTitleTranslation")}`)
    ?.addEventListener("command", () => {
      onPrefsEvents("setAutoTitleTranslation");
    });
  doc
    .querySelector(`#${makeId("skipChineseTitle")}`)
    ?.addEventListener("command", () => {
      onPrefsEvents("setSkipChineseTitle");
    });
  doc
    .querySelector(`#${makeId("overwriteTitleTranslation")}`)
    ?.addEventListener("command", () => {
...
function onPrefsEvents(type: string, fromElement: boolean = true) {
  ...
  switch (type) {
    case "setAutoTranslateSelection":
      addon.hooks.onReaderTabPanelRefresh();
      break;
    case "setAutoTitleTranslation":
      setPref(
        "enableAutoTitleTranslation",
    ...
    case "setSkipChineseTitle":
      setPref(
        "skipChineseTitle",
        (doc.querySelector(`#${makeId("skipChineseTitle")}`) as XUL.Checkbox)
          .checked,
      );
      break;
    case "setOverwriteTitleTranslation":
      setPref(
        "overwriteTitleTranslation",
