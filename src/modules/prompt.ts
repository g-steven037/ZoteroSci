  ...
  ztoolkit.Prompt.register([
    {
      ...
      callback: async (prompt) => {
        const selection = addon.data.translate.selectedText;
        const queue = (Zotero as any)[config.addonInstance].data.translate.queue;
        let task = queue.find(
        ...
        if (!task) {
          ...
          task = await (Zotero as any)[config.addonInstance].api.translate(selection);
          (Zotero as any)[config.addonInstance].data.translate.queue.push(task);
