...
import { hashTitle } from "../titleTranslation";
...
export class TranslationServices {
  ...
  public async runTranslationTask(
    ...
    if (!noCheckZoteroItemLanguage && task.itemId) {
      const item = Zotero.Items.getTopLevel([
        Zotero.Items.get(task.itemId) as unknown as Zotero.Item,
      ])[0];
        ...
        case "title":
          {
            if (item) {
              if (
                task.titleSourceHash &&
                hashTitle(String(item.getField("title") || "").trim()) !==
                  task.titleSourceHash
              ) {
                ztoolkit.log("Skipped stale title translation", item.id);
                break;
              }
